...
namespace hdist {
    ...
    struct Grid {
            ...
            for (size_t i = 0; i < length; ++i) {
                for (size_t j = 0; j < length; ++j) {
                    if (i == 0 || j == 0 || i == length - 1 || j == length - 1) {
                        this->operator[]({i, j}) = border_temp; // wtf?????????, assign value to data0, data1?
                    } else if (i == x && j == y) {
        ...
        double &operator[](std::pair<size_t, size_t> index) { // pair. READ
            return get_current_buffer()[index.first * length + index.second];
        }

        double &operator[](std::tuple<Alt, size_t, size_t> index) { // tuple WRITE
    ...
    UpdateResult update_single(size_t i, size_t j, Grid &grid, const State &state) { // doesn't change grid
        UpdateResult result{};
